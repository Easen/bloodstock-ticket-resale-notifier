...
function arrayDifferences(original: unknown[], updated: unknown[]): Ticket[] {
  ...
  return updatedJson
    .filter((x) => !originalJson.includes(x))
    .map((x) => JSON.parse(x));
}

export async function checkForNewTickets(): Promise<
...
> {
  ...
  return arrayDifferences(state, tickets) as unknown as Record<
    string,
    Partial<Ticket>
  >;
}
...
async function fetchTickets(url: string): Promise<Ticket[]> {
  ...
  const el = CssSelect.selectAll(
    "table.price-list.see-table.one-group-top tr.ticket",
    doc,
  );
  const tickets: Ticket[] = el.map((el) => {
    const text = el.children
      ...
      .map((textEl) => (textEl as Text[]).map((textEl) => textEl.data).join())
